...
void set_initial_values(mio::osecirvvs::Model<FP>& model)
{
    ...
    for (size_t i = 0; i < Secirvvs_NLP::tmax + 1; ++i) {
        ...
        model.parameters
            .template get<mio::osecirvvs::DailyFirstVaccination<FP>>()[{(mio::AgeGroup)0, mio::SimulationDay(i)}] =
            num_vaccinations;
        model.parameters
            .template get<mio::osecirvvs::DailyFullVaccination<FP>>()[{(mio::AgeGroup)0, mio::SimulationDay(i)}] =
            num_vaccinations;
    }
